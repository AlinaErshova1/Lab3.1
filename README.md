...
Программа ролучает 3 натуральных числa `X` , `Y` и `Z` - размеры трёхмерного массива.  Минимальное значение натурального числа это 1, а максимальное не ограниченно(в java максимальное - 2<sup>31-1</sup>) тип int.
...
Также для подзадачи 1 будет создан вспомогательный массив `m` размера `х`, в который будут записываться отсортированные числа. Тип данных - `double`.             
...
3.1 Создаю массив `m`, в который записываю отсортированные элементы 0-го столбца: сотритируем его по дробной части, если дробные части равны, сортируем по целой части и т.д. для оставшихся столбцов. Значения массива `m` приписываем массиву `а`. ("Обновляем массив" `а`, согласно условию сортировки)
...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < z; i++) {
            for (int j = 0; j < y; j++) {
                double[] m = new double[x]; //создание вспомогательного массива m
                for (int k = 0; k < x; k++) {
                    m[k] = a[i][k][j];
                }
                ...
                //здесь у нас отсортированный массив m
        ...
        out.println("Mассив с округленными эл.:");
        for (int i = 0; i<z; i++) {
            for (int j = 0; j < x; j++) {
                for (int k = 0; k < y; k++) {
